...
Publications on reproducibility can be found in all areas of scientific research. However, as \citet{Goodman341ps12} argue, the language and conceptual framework of research reproducibility \st{varies} \textbf{vary} significantly across the sciences, and there are no clear standards on reproducibility agreed upon by the scientific community as a whole. We consider recommendations from a variety of fields and determine the key aspects of reproducibility faced by scientists in different disciplines. 
...
Subsets of the sixteen tests can be invoked using the \pkg{tidyselect} helper functions \citep{R-tidyselect} in combination with the more limited \func{proj\_check\_some} function. These helper functions, which include \func{starts\_with}, \func{ends\_with}, \func{contains}, and \func{matches}, allow users to check for matches between a string or regular expression and a list of variables--in this case, searching for checks from the list in \func{list\_checks} whose names match a certain condition (i.e. ending with "root").
...
##   <chr>        <chr>                                <chr>    <dttm>             
## 1 package:pur~ <NA>                                 base::l~ 2020-10-06 21:01:26
## 2 package:for~ <NA>                                 base::l~ 2020-10-06 21:01:26
## 3 project_mic~ /Users/audreybertin/Documents/ferti~ readr::~ 2020-10-06 21:01:26
\end{verbatim}
\end{kframe}
\end{knitrout}
...
Much of the functionality in \pkg{fertile} is achieved by writing \href{https://en.wikipedia.org/wiki/Shim_(computing)}{shims}. \textbf{In computer programming generally, a shim is a function that intercepts a user's call, changes the arguments passed to it, and/or modifies the execution of the function in some way.} In the case of \pkg{fertile} specifically, shims are used to intercept the user's commands and perform various logging and path-checking tasks before executing the desired function. Our process is:
...
\end{alltt}
\begin{verbatim}
## /var/folders/v6/f62qz88s0sd5n3yqw9d8sb300000gn/T/Rtmp54U9ou/project_miceps
\end{verbatim}
\begin{alltt}
\hlstd{fs}\hlopt{::}\hlkwd{dir_ls}\hlstd{(temp_dir)} \hlopt{%>%} \hlkwd{head}\hlstd{(}\hlnum{3}\hlstd{)}
\end{alltt}
\begin{verbatim}
## /var/folders/v6/f62qz88s0sd5n3yqw9d8sb300000gn/T/Rtmp54U9ou/project_miceps/Blot_data_updated.csv
## /var/folders/v6/f62qz88s0sd5n3yqw9d8sb300000gn/T/Rtmp54U9ou/project_miceps/CS_data_redone.csv
## /var/folders/v6/f62qz88s0sd5n3yqw9d8sb300000gn/T/Rtmp54U9ou/project_miceps/Estrogen_Receptors.docx
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\begin{itemize} [noitemsep]
  \item \st{Expanding dependency management features to include R session information and package version numbers in addition to package names.}
  \item \st{Expanding code and documentation style features to analyze whether code has been properly commented in addition to checking for a README and tidy code style}.
