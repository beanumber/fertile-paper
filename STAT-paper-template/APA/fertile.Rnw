...
<<list-of-checks, echo = FALSE>>=
check_name <- c("has_proj_root()",
           "has_no_nested_proj_root()",
           "has_clear_build_chain()",
           "has_tidy_[media/images/code/raw_data/data/scripts]()",
           "has_only_used_files()",
           "has_readme()",
...
description <- c(
  "Checks to make sure a single .Rproj file is found in the root directory of the project.",
  "Checks to make sure there are no nested .Rproj files in subfolders of the project.",
  "Looks for indication of a clear order in which to run R scripts.",
  "There are six different 'has_tidy_*()' functions. Each checks to make sure that no files of its given type (images, code, data, etc.) are located in the root directory of the project",
  "Ensures that all files located in the project directory are being referenced or created by other files in the project.",
  "Checks to make sure there is a README file in the root directory.",
  "Checks to ensure that all code files contain an adequate number of comments (>10% of lines).",
  "Checks to make sure that no absolute paths are referenced in code.",
  "Checks to make sure that no paths in code lead outside the project directory.",
  "Checks to make sure code does not contain uncontrolled randomness.",
  "Examines code to determine whether it conforms to 'tidy' style guide recommendations.")

library(magrittr)
tibble::tibble("Check" = check_name, "Functionality" = description) %>%
  kableExtra::kbl( 
    caption = "Table illustrating the list of check available in fertile. ",
    format = "latex",
    booktabs = TRUE
  ) %>%
  kableExtra::kable_styling(latex_options = c("striped"), full_width = TRUE) %>%
...
<<read-csv, message=FALSE, results = 'hide'>>=
library(purrr)
library(forcats)
...
One of the challenges with ensuring that work is reproducible is \textcolor{blue}{\st{the issue of}} dependencies. Many data analysis projects reference a variety of \R packages in their code. When such projects are shared with other users who may not have the required packages downloaded, it can cause errors that prevent the project from running properly. 
