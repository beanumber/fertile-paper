...
Data-based research is considered fully reproducible when the requisite code and data files
produce identical results when run by another analyst. 

As research is becoming increasingly data-driven, and because knowledge can be shared worldwide so rapidly, reproducibility is critical to the advancement of scientific knowledge. Academics around the world have recognized this, and publications and discusions addressing reproducibility appear to have increased in the last several years (\cite{sep-scientific-reproducibility}, \cite{plos-biology}, \cite{bioessays-gosselin}, \cite{eisner-reproducibility}, \cite{engineering-reproducibility}). 
...
outputs from one step serving as inputs into the next. This is a common suggestion within the computing community, originating as part of the Unix philosophy \cite{unix}.
\end{itemize}
...
Broman reiterates the need for clear naming and file separation while sharing several additional suggestions: keep the project contained in one directory, use relative paths, and include a \cmd{README} (\cite{broman}).
...
\item[4] Randomness is accounted for:
  ...
  \end{itemize}
\item[5] Code conforms to \pkg{tidyverse} style:
  \begin{itemize}
  \item Although not discussed in any of the literature mentioned previously, we believe \pkg{tidyverse} integration to be very important. Code conformation to \pkg{tidyverse} style helps ensure that analyses meeting \pkg{fertile}'s recommendations will be able to integrate seamlessly with packages from the \pkg{tidyverse}, an ever-expanding collection of packages designed to operate together with one another.
  \end{itemize}
\end{itemize}

Much of the available literature focuses on file structure, organization, and naming, and \pkg{fertile}'s features are consistent with this. The ideal file structure is not agreed upon by academics, though there are some recommendations focused on \R that are available. \cite{marwick2018packaging} provides the framework for file structure that \pkg{fertile} is based on: a structure similar to that of an \R package (\cite{hadley-packages}, \cite{coreteam-extensions}), with an \cmd{R} folder, as well as \cmd{data}, \cmd{data-raw}, \cmd{inst}, and \cmd{vignettes}.

\subsection{R Packages and Other Software}

Much of the written work discussed in Section \ref{papers} is highly generalized, written to be applicable to users working with a variety of statistical software programs. Because all statistical software programs operate differently, these recommendations are inherently vague and difficult to implement, particularly to new analysts who are relatively unfamiliar with their software. 

Thus, attempts to address reproducibility that are specific to certain software programs are more likely to be successful. \R, due to its accessibility and popularity as a tool for statistical analysis, is an ideal candidate such recommendations.

Acknowledging this, researchers and members of the \R community have created a small body of \R packages that focus on research reproducibility, although not all of them are available on CRAN:

\pkg{rrtools} (\cite{R-rrtools}) addresses some of the issues discussed in \cite{marwick2018packaging} by creating a basic \R package structure for a data analysis project and implementing a basic \func{testthat::check} functionality. The \pkg{orderly} (\cite{R-orderly}) package also focuses on file structure, requiring the user to declare a desired project structure (typically a step-by-step structure, where outputs from one step are inputs into the next) at the beginning and then creating the files necessary to achieve that structure. \pkg{workflowr}'s (\cite{R-workflowr}) functionality is based around version control and making code easily available online. It works to generate a website containing time-stamped, versioned, and documented results. \pkg{checkers} (\cite{R-checkers}) allows you to create custom checks that examine different aspects of reproducibility. \pkg{packrat} (\cite{R-packrat}) is focused on dependencies, creating a packaged folder containing a project as well as all of its dependencies, so that projects dependent on lesser-used packages can be easily shared across computers. \pkg{drake} (\cite{R-drake}) analyzes workflows, skip steps where results are up to date, and provides evidence that results match the underlying code and data. Lastly, the \pkg{reproducible} (\cite{R-reproducible}) package focuses on the concept of caching: saving information so that projects can be run faster each time they are re-completed from the start.


Many of these packages are narrowly tailored, with each effectively addressing a small component of reproducibility: file structure, modularization of code, version control, etc. These packages often  succeed in their area of focus, but at the cost of accessibility to a wider audience. Their functions are often quite complex to use, and many steps must be completed to achieve the required reproducibility goal. This cumbersome nature means that most reproducibility packages currently available are not easily accessible to users near the beginning of their \R journey, nor particularly useful to those looking for quick and easy reproducibility checks.

A much more effective way of realizing widespread reproducibility is to make the process for doing so simple enough that it takes little to no conscious effort to implement. 

Several continuous integration code development tools external to R attempt to do this, providing a way to address different aspects of reproducibility with minimal code. For example, \pkg{Wercker}, a command line tool that leverages Docker, enables users to test whether their projects will successfully compile when run on a variety of operating systems without access to the user's local hard drive \cite{wercker}. \pkg{Travis CI} is integrated with GitHub and can be configured to do similar checks on projects hosted in repositories \cite{travis}. \pkg{Circle CI}, another popular continuous integration tool, can also be used to check R code \cite{circle}. 

However, while these tools can be useful in some circumstances, they are generalized so as to be useful to the widest audience. As a result, their checks are not designed to be R-specific, which makes them sub-optimal for users looking to address reproducibility issues involving features specific to the R programming language, such as package installtion and seed setting.


\section{Methods}\label{sec3}

\pkg{fertile} attempts to address these gaps in existing software by providing a simple, easy-to-learn reproducibility package that, rather than focusing intensely on a specific area, provides some information about a wide variety of aspects influencing reproducibility. \pkg{fertile} also allows for flexibility, offering benefits to users at any stage in the data analysis workflow, and provides R-specific features, which address certain aspects of reproducibility that can be missed by external project development software.
...
Users can access the log recording their commands at any time via the \func{log\_report} function:
...
In order to produce \pkg{fertile}'s warnings and reports, we take advantage of a function manipulation technique called \cmd{shimming}, which consists of the following steps:

\begin{itemize}
\item[1] We begin by choosing a function that exists in the R universe that we are interested in recording user interaction with. Popular functions linked to only one package, such as \cmd{read\_csv()} from \pkg{readr}, are ideal candidates.
\item[2] Then, we creating a function with an identical name to this function within our own package.
\item[3] We write this new function so that it calls the original function, captures the arguments that are passed in, and saves information of which function was called and which arguments were passed to it to a hidden log file. This process looks the same to the user as if they were calling the original function.
\end{itemize}

\pkg{fertile} shims many common functions, including those that read in a variety of data types, write data, and load packages. This allows us to track functions that may commonly be misused. For example, any time a user calls \cmd{read\_csv()} in their code, \pkg{fertile} captures this use, identifies the type of path the user supplied to the function, and uses that information to determine whether a warning is necessary. This works both proactively and retroactively, as the shimmed functions written in \pkg{fertile} are activated both when the user is coding interactively and when a file containing code is rendered.

In order to ensure that the \pkg{fertile} versions of functions are always favored over their original forms when called, we manipulate the \R \cmd{search()} path so that \pkg{fertile} is always located in the first position using a shim of the \cmd{library()} function. In the \pkg{fertile} version of \cmd{library()}, we detach \pkg{fertile} from the search path, load the requested package, and then re-attach \pkg{fertile}. This makes certain that when a user executes a command, \R will check \pkg{fertile} for a matching function before considering other packages, enabling \pkg{fertile} to successfully record--and therefore report on--user behavior.   
...
For users focused on file paths, the \func{check\_path()} function analyzes a vector of paths (or a single path) to determine whether there are any absolute paths or paths that lead outside the project directory.

<<path checks, error = TRUE>>=
...
is_image_file(fs::path(proj_dir, "mice.csv"))
...
\pkg{fertile} is wide in scope. The package's simplicity enables users of any background to take advantage of its features, and its big-picture design gives \pkg{fertile} the potential to provide benefits across a variety of disciplines. 
...
She reads in the data, makes the graph, and knits her \cmd{.Rmd} file. It compiles successfully, so she submits the assignment. The next day, she receives an email from her professor saying that her assignment failed to compile and that she needs to make changes and try again.
...
To test \pkg{fertile}'s effectiveness, we began an initial randomized control trial of the package on an introductory undergraduate data science course at Smith College in Spring 2020. Unfortunately, however, the trial had to be postponed after the COVID-19 pandemic significantly altered the experimental conditions at the midpoint of testing. \\

The experiment was structured as follows:

\begin{itemize}

\item Students are given a form at the start of the semester asking whether they consent to participate in a study on data science education. In order to successfully consent, they must provide their system username, collected through the command \cmd{Sys.getenv("LOGNAME")}. Since these individuals each username belongs to are often easily identifiable, each email address is then transformed into a hexadecimal string via a hashing algorithm in to maintain privacy. 
...
\item The students are then asked to download a package titled the same as the course number of the class: \pkg{sds192}, which was custom created for the purpose of this trial. It leverages an \cmd{.onAttach} function to scan the R environment and collect the username of the user who is loading the package and run it through the same hashing algorithm as used previously. It then identifies whether that user belongs to the experimental or the control group. Depending on the group they are in, they receive a different version of the package on loading:

\item The experimental group receives the basic \pkg{sds192} package, which consists of some data sets and RMarkdown templates necessary for completing homework assignments and projects in the class, but they also have \pkg{fertile} installed and loaded silently in the background. The package's proactive features are enabled without their knowledge, and they can receive warning messages when they use absolute or non-portable paths or attempt to change their working directory. The control group receives only the basic \pkg{sds192} package, including its data sets and RMarkdown templates. They do not have \pkg{fertile} installed and loaded in the background and do not receive proactive warning messages. All students from both groups then use their version of the package throughout the semester on a variety of projects.

\item In order to test \pkg{fertile}'s effectiveness, both groups are given a short quiz on different components of reproducibility that are intended to be taught by \pkg{fertile} at both the beginning and end of the semester. Their scores are then compared to see whether one group learned more than the other group or whether their scores were essentially equivalent. 

\end{itemize}

Although the experiment was unsuccessful in its first attempt, we hope to run the same trial again and gather data on \pkg{fertile}'s effectiveness once the higher education environment returns to a relatively stable state. 
...
The process of ensuring true reproducibility is highly complex and challenging. There are a significant number of components, many of which are described in \ref{sec:lit}, which must all be in place for work to be reproducibile. \pkg{fertile}, in its current version, addresses the vast majority of these in some way. However, there are several areas where further development to extend the available features of the package would be beneficial. These include the following:
