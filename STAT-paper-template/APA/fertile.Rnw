...
Much of the discussion around reproducibility is also generalized, written to be applicable to users working with a variety of statistical software programs. Because all statistical software programs operate differently, generalized recommendations regarding reproducibility are often shallow and vague. While they provide useful guidelines, they can often be difficult to implement, particularly to new analysts who are unsure of how to apply such recommendations within the software programs they are using. Thus, reproducibility recommendations tailored to specific software programs are more likely to be adopted.

In this paper, we focus on reproducibility in the \R programming language, concentrating specifically on projects that use the RStudio integrated development environment. \R is an ideal candidate for reproducibility recommendations because of its accessibility and popularity as a tool for statistical analysis.

A small body of \R packages which focus on reproducibility have been published in recent years. Much of this work is narrowly tailored, with each package effectively addressing a small component of reproducibility: file structure, modularization of code, version control, etc. These packages often  succeed in their area of focus, but at the cost of accessibility to a wider audience. They are often difficult to learn and operate, providing a barrier to entry for less-experienced data analysts. This can lead users to become discouraged and abandon attempts to achieve reproducibility.

A much more effective way of realizing widespread reproducibility is to make the process for doing so simple enough that it takes little to no conscious effort to implement. However, there appear to be no existing tools that fit this model--catering to analysts looking for a simple, easy-to-implement, and fast way to obtain a broad overview of their projects' reproducibility. To address this, we present \pkg{fertile}, a low barrier-to-entry package focusing on common mistakes programmers make while conducting data science research in R.
...
Reproducibility is essential to the advancement of knowledge in all fields of scientific research. Researchers acknowledge this; publications and discussions focusing on reproducibility appear to have increased in frequency over the last several years (\cite{sep-scientific-reproducibility}, \cite{plos-biology}, \cite{bioessays-gosselin}, \cite{eisner-reproducibility}, \cite{engineering-reproducibility}).

Much of the available literature is focused on methods for achieving reproducibility in specific disciplines and some of their recommendations are generalizable to all areas of scientific research. There is also work focusing specifically on reproducibility within the \R community. Some of this work is in the form of academic papers, while other literature comes from blog posts, websites, and \R packages, some of which are compiled by notable R-focused developers and researchers. In this section, we will discuss the reproducibility recommendations from both the formal and informal publications, which together have influenced the development of \pkg{fertile}.


\subsection{Previous Work}

\cite{Goodman341ps12} argue that the language and conceptual framework of research reproducibility varies across the sciences. There are no clear standards agreed upon across fields.
...
There are also several development tools external to R that focus on continuous integration and can be configured to address different aspects of reproducibility. \pkg{Wercker}, a command line tool that leverages Docker, enables users to test whether their projects will successfully compile when run on a variety of operating systems without access to the user's local hard drive \cite{wercker}. \pkg{Travis CI} is integrated with GitHub and can be configured to do similar checks on projects hosted in repositories \cite{travis}. \pkg{Circle CI}, another popular continuous integration tool, can also be used to check R code \cite{circle}. 

However, while these tools can be useful in some circumstances, they are generalized so as to be useful to the widest audience. As a result, their checks are not designed to be R-specific, which makes these tools sub-optimal for users looking to address reproducibility issues involving features specific to the R programming language, such as package installtion and seed setting.

\section{Methods}\label{sec3}

\pkg{fertile} addresses these gaps by providing a simple, easy-to-learn reproducibility package that, rather than focusing intensely on a specific area, provides some information about a wide variety of aspects influencing reproducibility. The package also provides flexibility, offering benefits to users at any stage in the data analysis workflow, and R-specific features, which address certain aspects of reproducibility that can be missed by external project development software.
...
To test \pkg{fertile}'s effectiveness, we began an initial randomized control trial of the package on an introductory undergraduate data science course at Smith College in Spring 2020, comparing one group of students that received the package functions in their \R environment to one that did not. Unfortunately, however, the trial had to be postponed after the COVID-19 pandemic significantly altered the experimental conditions at the midpoint of testing. 

We hope to continue this experimentation and gather data on \pkg{fertile}'s effectiveness once the higher education environment returns to a relatively stable state. 
