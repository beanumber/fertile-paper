...
**fertile** also contains a logging functionality, which records commands with the potential to affect reproducibility that are run in the console. The package focuses mostly on package loading and file opening, notes which function was used, the path or package it referenced, and the timestamp at which that event happened.
...
The `log_clear()` function allows for management of the log, enabling users to erase it and start over at any time.
