...
Unlike with many other R packages focused on reproducibility that are currently available, the features of **fertile** can be accessed almost effortlessly, making it easy for data analysts of all skill levels and backgrounds to gain a better understanding of how to make their work reproducible. Many of **fertile**'s features can be accessed in only two lines of code and using minimal arguments, and some of the proactive features can be accessed purely upon loading the package. As a result, it is beneficial to a wide audience--simple enough to be used by students in an introductory data science course, but still helpful to experienced analysts.
