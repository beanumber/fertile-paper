...
These proactive warning features are activated immediately after loading the **fertile** package and require no additional effort by the user to access.  


## Retroactive Use

Retroactively, **fertile** analyzes potential obstacles to reproducibility in an RStudio Project (i.e., a directory that contains an `.Rproj` file). The package considers several different aspects of the project which may influence reproducibility, including the directory structure, the analyst's use of file paths, and whether randomness is used thoughtfully.  
...
Subsets of the fifteen tests can be invoked using the select helper functions from **dplyr** in combination with the `proj_check_some()` function. 
...
The log, if not managed, can grow very long over time. For users who do not desire such functionality, the `log_clear()` provides a way to erase the log and start it over.
...
In addition to the interactive log, users can access something called the render log, which contains much of the information used to create the output for `proj_analyze()` using the function `render_log_report()`.
...
**fertile** is meant to be educational, providing informative error messages showing how the ways in which the mistakes the user is making are problematic, as well as sharing recommendations on how to fix them. The package is designed in this way so as to promote a greater understanding of reproducibility concepts in its users, with the goal of increasing the overall awareness and understanding of reproducibility in the R community. 

The package has very low barriers to entry, making it accessible to users with various levels of background knowledge. Unlike with many other R packages focused on reproducibility that are currently available, the features of **fertile** can be accessed almost effortlessly. Many of the retroactive features can be accessed in only two lines of code requiring  minimal arguments, and some of the proactive features can be accessed with no additional effort beyond loading the package. This, in combination with the fact that **fertile** does not focus on one specific area of reproducibility, instead covering (in less detail) a wide variety of topics, means that **fertile** makes it easy for data analysts of all skill levels to quickly gain a better understanding of the reproducibility of the work. 
