...
In addition to the primary reproducability analsis functions present in \cmd{check}, \cmd{proj\_analyze()}, and the interactive warning system, several useful utility functions are made available to \pkg{fertile} users. Among other things, these include functions to check the type of a file and a way to create a copy of a project in a temp directory.
